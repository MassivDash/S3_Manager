use aws_sdk_s3::{Client, model::{ObjectIdentifier, Delete, Object}};
use std::error::Error;
...
pub async fn delete_folders(window: Window, keys: Vec<FilesToDelete>) -> bool {
    ...
    for key in keys {
        remove_folder_with_contents(&client, &key.bucket_name, &key.key).await.unwrap();
    }
...
async fn remove_folder_with_contents(client: &Client, bucket: &str, prefix: &str) -> Result<(), Box<dyn Error>> {
